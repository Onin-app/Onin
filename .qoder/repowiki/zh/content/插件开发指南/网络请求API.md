...
async function postData() {
  try {
    ...
    if (isHttpError(error)) {
      ...
      console.error('权限不足，请检查manifest.json中的network权限');
    } else {
...
**错误信息**：`Permission denied for URL: https://api.example.com. Please add it to the 'permissions.network' in your manifest.json.`

**解决方案**：
- 检查`manifest.json`中的`permissions.network`配置
- 确保URL格式正确（包含协议）
- 使用正确的通配符语法
