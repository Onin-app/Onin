...
- [index.ts](file://plugins-sdk/src/index.ts) - *更新了API导出结构*
- [permissions.ts](file://plugins-sdk/src/types/permissions.ts) - *新增权限类型定义*
</cite>

## 更新摘要
**变更内容**
- 更新“权限控制模型”章节，反映权限系统重构为命名空间结构
- 修正网络权限声明格式，从扁平化数组改为对象结构
- 更新错误提示信息，指向新的 `permissions.http.allowUrls` 路径
- 修改前端API调用方式，统一通过 `baize` 命名空间访问模块
- 新增对 `http` 模块便捷方法（如 `get`, `post`）的说明
- 移除过时的 `baize.request` 直接调用示例，替换为 `baize.http.request` 形式
...
### `baize.http.request` API 设计

`baize.http.request` API 为插件提供了一个统一的网络请求接口，支持GET、POST等HTTP方法，并允许指定请求头、超时时间等选项。

API设计包含 `RequestOptions` 接口定义请求参数，`Response` 接口定义响应结构。SDK提供了 `request` 函数，接受 `RequestOptions` 并返回 `Promise<Response>`。此外，还提供了便捷方法如 `get`, `post`, `put`, `delete`。
...
- [request.ts](file://plugins-sdk/src/api/request.ts#L0-L144)
- [baize-request-api-design.md](file://baize-request-api-design.md#L0-L199)
- [index.ts](file://plugins-sdk/src/index.ts#L1-L70)

## 权限控制模型

### 网络权限声明

插件必须在 `manifest.json` 文件中声明其需要访问的网络域名。权限声明位于 `permissions.http` 对象中，支持精确匹配和通配符匹配。

新的权限结构采用命名空间形式：
- **`permissions.http.enable`**: 布尔值，启用HTTP权限开关
- **`permissions.http.allowUrls`**: 字符串数组，包含所有允许访问的域名或模式

通配符 `*` 只能用于子域名，例如 `https://*.github.com` 可以匹配 `api.github.com` 但不能匹配 `github.com`。权限检查在Rust后端进行，确保无法被绕过。

当插件发起网络请求时，后端会加载其 `manifest.json`，检查请求URL是否与声明的权限匹配。如果匹配失败，则返回 `PermissionDeniedError`，错误信息明确指向 `permissions.http.allowUrls` 配置项。
...
插件JS->>SDK : baize.http.request(options)
...
- [permissions.ts](file://plugins-sdk/src/types/permissions.ts#L46-L46)

## 跨环境兼容策略

### 统一的API调用方式

为了提高协议一致性，所有SDK API均通过 `baize` 命名空间下的模块进行调用。`plugins-sdk` 的入口文件 `index.ts` 导出了 `http`, `storage`, `notification`, `command` 等模块对象。
...
// 注册命令处理器
baize.command.registerCommandHandler('my-command', async () => {
  // 处理逻辑
});
```

此变更优化了API的导入与导出机制，封装了底层通信细节，使插件开发者能够以一致的方式使用各类功能。

**Section sources**
- [index.ts](file://plugins-sdk/src/index.ts#L1-L70)
- [request.ts](file://plugins-sdk/src/api/request.ts#L182-L190)

## 总结

本文档详细阐述了插件通信协议的实现机制，包括自定义协议处理、双向通信、SDK实现及权限控制。重点更新了因权限系统重构和API封装优化所带来的变化，确保文档与最新代码状态保持一致。开发者应使用 `baize.http.request` 等新式调用方式，并在 `manifest.json` 中正确配置 `permissions.http` 对象以获得网络访问权限。