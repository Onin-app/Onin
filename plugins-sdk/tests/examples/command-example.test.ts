  ...
  describe('PluginState', () => {
    ...
    describe('data commands', () => {
      ...
      it('should handle data.get command', async () => {
        await pluginState.handleCommand('data.set', { key: 'test', value: 'hello' });
        
        const result = await pluginState.handleCommand('data.get', { key: 'test' });
      ...
      it('should handle data.list command', async () => {
        await pluginState.handleCommand('data.set', { key: 'key1', value: 'value1' });
        await pluginState.handleCommand('data.set', { key: 'key2', value: 'value2' });
      ...
      it('should handle data.delete command', async () => {
        ...
        const result = await pluginState.handleCommand('data.delete', { key: 'test' });
        ...
        const getResult = await pluginState.handleCommand('data.get', { key: 'test' });
    ...
    describe('plugin commands', () => {
      it('should handle plugin.status command', async () => {
        await pluginState.handleCommand('data.set', { key: 'test', value: 'value' });
      ...
      it('should handle plugin.reset command', async () => {
        await pluginState.handleCommand('data.set', { key: 'test', value: 'value' });
        await pluginState.handleCommand('settings.update', { settings: { theme: 'light' } });
        ...
        const statusResult = await pluginState.handleCommand('plugin.status', {});
    ...
    describe('complex workflows', () => {
      it('should handle complete data management workflow', async () => {
        // Set multiple values
        await pluginState.handleCommand('data.set', { key: 'user1', value: { name: 'John', age: 30 } });
        await pluginState.handleCommand('data.set', { key: 'user2', value: { name: 'Jane', age: 25 } });
        ...
        // Get specific user
        const getResult = await pluginState.handleCommand('data.get', { key: 'user1' });
        ...
        // Verify deletion
        const finalListResult = await pluginState.handleCommand('data.list', {});
      ...
      it('should handle settings and status workflow', async () => {
        ...
        const initialStatus = await pluginState.handleCommand('plugin.status', {});
        expect(initialStatus.dataCount).toBe(0);
        expect(initialStatus.settings.theme).toBe('dark');
        
        // Add some data and update settings
        await pluginState.handleCommand('data.set', { key: 'config', value: 'test' });
        await pluginState.handleCommand('settings.update', { settings: { theme: 'light', debug: true } });
        
        // Check updated status
        const updatedStatus = await pluginState.handleCommand('plugin.status', {});
        ...
        // Verify reset
        const resetStatus = await pluginState.handleCommand('plugin.status', {});
