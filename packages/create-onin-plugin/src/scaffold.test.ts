...
const adapterTemplateDirs: Record<
  Framework,
...
test('scaffoldPlugin creates a vanilla JavaScript project', async () => {
  ...
  try {
    ...
    assert.match(
      await readFile(join(targetDir, 'src', 'main.js'), 'utf8'),
      /Smoke Plugin/,
    );
    assert.match(
...
test('scaffoldPlugin creates a react JavaScript project', async () => {
  ...
  try {
    ...
    assert.match(
      await readFile(join(targetDir, 'src', 'main.jsx'), 'utf8'),
      /Smoke Plugin/,
    );
    assert.match(
      await readFile(join(targetDir, 'src', 'main.jsx'), 'utf8'),
    ...
    assert.match(
      await readFile(join(targetDir, 'vite.config.js'), 'utf8'),
      /plugin-react/,
    );
  } finally {
    await rm(tempDir, { recursive: true, force: true });
...
test('scaffoldPlugin creates a vue JavaScript project', async () => {
  ...
  try {
    ...
    assert.match(
      await readFile(join(targetDir, 'src', 'App.vue'), 'utf8'),
      /pluginName/,
    );
    assert.match(
      await readFile(join(targetDir, 'src', 'main.js'), 'utf8'),
