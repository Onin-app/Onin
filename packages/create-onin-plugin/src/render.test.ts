...
test('copyTemplateDir copies nested template files and keeps nested skipped names', async () => {
  ...
  try {
    ...
    await writeFile(
      join(sourceDir, 'package.fragment.json'),
      '{"ignored":true}',
      'utf8',
    );
    await writeFile(
      join(sourceDir, 'src', 'package.fragment.json.tpl'),
      '__PLUGIN_NAME__',
      'utf8',
    );
    await writeFile(
      join(sourceDir, 'src', 'main.ts.tpl'),
    ...
    assert.equal(
      await readFile(join(targetDir, 'src', 'package.fragment.json'), 'utf8'),
      'Sample Plugin',
    );
    assert.equal(
...
test('renderPackageJson injects release configuration when withRelease is true', async () => {
  ...
  try {
    ...
    await writeFile(adapterPath, JSON.stringify({}), 'utf8');
