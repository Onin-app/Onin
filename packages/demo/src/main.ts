...
log('Onin SDK Demo initialized');
log(
  'Available APIs: lifecycle, storage, notification, clipboard, http, fs, dialog, command, settings, scheduler',
);

// Test Lifecycle API
document
  .getElementById('test-lifecycle')
  ...
  ?.addEventListener('click', async () => {
    ...
    try {
      await notification.show({
        ...
        body: 'This is a test notification from Onin SDK Demo',
      });
...
// Test Clipboard API
document
  .getElementById('test-clipboard')
  ?.addEventListener('click', async () => {
    log('\n=== Testing Clipboard API ===');
    try {
      const testText = 'Hello from Onin SDK Demo!';
