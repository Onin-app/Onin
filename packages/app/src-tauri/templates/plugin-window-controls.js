(function () {
  // 注入运行时环境信息（窗口模式）
  // 从 URL 获取 pluginId，窗口 label 格式为 plugin_xxx_yyy -> xxx.yyy
  const currentLabel =
    window.__TAURI_INTERNALS__?.metadata?.currentWindow?.label || "";
  const extractedPluginId = currentLabel
    .replace("plugin_", "")
    .replace(/_/g, ".");

  window.__ONIN_RUNTIME__ = {
    mode: "window",
    pluginId: extractedPluginId || "unknown",
    version: "0.1.0",
    mainWindowLabel: "main",
  };

  // 保留兼容性
  window.__PLUGIN_ID__ = extractedPluginId;

  console.log("[Plugin Window] Runtime injected:", window.__ONIN_RUNTIME__);
        ...
        getCurrent: function () {
          ...
          return {
            ...
            close: async function () {
              return window.__TAURI_INTERNALS__.invoke("plugin_close_window", {
                label,
              });
            },
            minimize: async function () {
              return window.__TAURI_INTERNALS__.invoke(
                "plugin_minimize_window",
                { label },
              );
            },
            maximize: async function () {
              return window.__TAURI_INTERNALS__.invoke(
                "plugin_maximize_window",
                { label },
              );
            },
            ...
            isMaximized: async function () {
              return window.__TAURI_INTERNALS__.invoke("plugin_is_maximized", {
                label,
              });
            },
            show: async function () {
              return window.__TAURI_INTERNALS__.invoke("plugin_show_window", {
                label,
              });
            },
            setFocus: async function () {
              return window.__TAURI_INTERNALS__.invoke("plugin_set_focus", {
                label,
              });
            },
          };
          ...
          element.addEventListener("mousedown", async (e) => {
            ...
            if (e.button === 0 && !e.target.closest("button")) {
              ...
              try {
                ...
                console.error(
                  "[Plugin Window] Failed to start dragging:",
                  error,
                );
      ...
      if (backBtn) {
        backBtn.addEventListener("click", async () => {
          try {
            ...
            console.error(
              "[Plugin Window] Failed to switch to inline mode:",
              error,
            );
