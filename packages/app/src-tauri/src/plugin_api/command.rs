...
pub struct CommandExecutionStore(
    pub Mutex<HashMap<String, tokio::sync::oneshot::Sender<CommandExecutionResult>>>,
);
...
async fn execute_headless_command(
    ...
    // 执行指令
    let result = manager
        .execute_command(plugin_id.clone(), command_name.to_string(), args_json)
        .await?;
