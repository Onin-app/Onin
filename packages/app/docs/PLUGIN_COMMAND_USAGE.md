...
```typescript
import onin from "@onin/plugin-sdk";

// 注册指令处理器
onin.registerCommandHandler(async (command: string, args: any) => {
...
// 注册指令处理器
onin.registerCommandHandler(async (command, args) => {
  switch (command) {
    case "system_info":
      return {
        timestamp: Date.now(),
...
<html>
  <head>
    ...
    <script type="module">
      ...
      // 注册指令处理器
      await onin.registerCommandHandler(async (command, args) => {
...
4. 调用插件注册的 `__ONIN_COMMAND_HANDLER__` 函数
5. 返回执行结果
