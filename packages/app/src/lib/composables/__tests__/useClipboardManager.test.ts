...
function createClipboardEvent(
  items: Array<{ kind: string; type: string; data?: string }>,
): ClipboardEvent {
  const dataTransfer = {
    items: items.map((item, i) => ({
      ...
      type: item.type,
      getAsFile: () =>
        item.kind === "file" ? new File([""], `file-${i}.txt`) : null,
...
describe("useClipboardManager", () => {
  ...
  it("removeFile should hide showAllFiles when only 1 left", () => {
    const cm = useClipboardManager();
    cm.state.attachedFiles = [new File([""], "a.txt"), new File([""], "b.txt")];
  ...
  it("autoPasteClipboard should populate files from invoke result", async () => {
    ...
    mockInvoke
      .mockResolvedValueOnce({
        files: [
          { path: "/tmp/file.txt", name: "file.txt", is_directory: false },
        ],
  ...
  it("autoPasteClipboard should handle directory files", async () => {
    ...
    mockInvoke
      .mockResolvedValueOnce({
        files: [{ path: "/tmp/mydir", name: "mydir", is_directory: true }],
        timestamp: Math.floor(Date.now() / 1000),
      })
      .mockResolvedValueOnce({ auto_paste_time_limit: 30 });
  ...
  it("autoPasteClipboard should populate image from invoke result", async () => {
    const cm = useClipboardManager();
    const fakeBase64Image =
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==";

    // Mock global fetch for base64 data url
    ...
    try {
      ...
      expect(cm.state.attachedFiles).toHaveLength(1);
      expect(cm.state.attachedFiles[0].name).toBe("image.png");
      expect(cm.state.attachedFiles[0].type).toBe("image/png");
      expect(cm.state.attachedText).toBe("");
    } finally {
      // Restore fetch
