...
describe('usePluginList', () => {
  ...
  it('togglePlugin should show error notification on failure', async () => {
    ...
    expect(console.error).toHaveBeenCalled();
    const showNotificationCall = mockInvoke.mock.calls.find(call => call[0] === 'show_notification');
  ...
  it('setupListeners should register listeners and return cleanup', async () => {
    ...
    expect(mockListen).toHaveBeenCalledWith('plugin-installed', expect.any(Function));
    expect(mockListen).toHaveBeenCalledWith('plugin-settings-schema-registered', expect.any(Function));
    expect(mockListen).toHaveBeenCalledWith('plugin-init-error', expect.any(Function));
    expect(mockListen).toHaveBeenCalledWith('plugin-init-success', expect.any(Function));
