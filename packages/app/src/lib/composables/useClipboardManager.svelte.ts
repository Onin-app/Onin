...
export function useClipboardManager(): ClipboardManagerReturn {
     ...
     * 获取匹配的命令
     */
    const getMatchedCommandsForAttachments = (originAppList: LaunchableItem[]): LaunchableItem[] => {
        return getMatchedCommands(originAppList, state.attachedText, state.attachedFiles);
    };
