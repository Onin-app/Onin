<script lang="ts">
  ...
  // 获取某个组在某行某列的扁平索引
  function getFlatIndexForGroupRowCol(groupIndex: number, row: number, col: number): number | null {
  ...
  export function handleKeyDown(e: KeyboardEvent) {
    ...
    switch (e.key) {
      ...
      case "ArrowDown":
        ...
        {
          // 先尝试在同组内向下一行
          let nextIndex = getFlatIndexForGroupRowCol(pos.groupIndex, pos.row + 1, pos.col);
          if (nextIndex === null) {
            // 没有下一行，尝试找同一行更靠左的位置
            for (let c = pos.col - 1; c >= 0; c--) {
              nextIndex = getFlatIndexForGroupRowCol(pos.groupIndex, pos.row + 1, c);
          ...
          if (nextIndex === null) {
            ...
            for (let g = pos.groupIndex + 1; g < data.groups.length; g++) {
              ...
              if (nextIndex !== null) break;
              // 如果该列不存在，尝试最后一个
              const lastColInGroup = Math.min(pos.col, data.groups[g].emojis.length - 1);
      ...
      case "ArrowUp":
        ...
        {
          // 先尝试在同组内向上一行
          let prevIndex = getFlatIndexForGroupRowCol(pos.groupIndex, pos.row - 1, pos.col);
          if (prevIndex === null && pos.row > 0) {
            // 上一行存在但该列不存在，找靠左的
            for (let c = pos.col - 1; c >= 0; c--) {
              prevIndex = getFlatIndexForGroupRowCol(pos.groupIndex, pos.row - 1, c);
          ...
          if (prevIndex === null) {
            ...
            for (let g = pos.groupIndex - 1; g >= 0; g--) {
              ...
              const lastRow = Math.floor((group.emojis.length - 1) / gridColumns);
              prevIndex = getFlatIndexForGroupRowCol(g, lastRow, pos.col);
              if (prevIndex !== null) break;
              // 如果该列不存在，取最后一行最后一个
              prevIndex = getFlatIndexForGroupRowCol(g, lastRow, (group.emojis.length - 1) % gridColumns);
              if (prevIndex !== null) break;
            }
          }
          if (prevIndex !== null) {
