...
function makeItem(
  name: string,
  matches: LaunchableItem["matches"],
): LaunchableItem {
  return {
    name,
    matches,
    ...
    source: "Command",
    keywords: [],
    type: matches && matches.length > 0 ? "Match" : "Function",
  };
}
...
describe("checkCommandMatch - file/image types", () => {
  it("matches image files with image type", () => {
    ...
    expect(
      checkCommandMatch(imageItem, "", [makeFile("photo.png", "image/png")]),
    ).toBe(true);
    expect(
      checkCommandMatch(imageItem, "", [
  ...
  it("matches file type with wildcard extension", () => {
    ...
    expect(
      checkCommandMatch(anyFileItem, "", [
        makeFile("test.xyz", "application/octet-stream"),
      ]),
    ).toBe(true);
  });
  ...
  it("matches files without MIME type via extension fallback", () => {
    ...
    expect(checkCommandMatch(extItem, "", [makeFile("script.js", "")])).toBe(
      true,
    );
    expect(checkCommandMatch(extItem, "", [makeFile("script.ts", "")])).toBe(
      false,
    );
  });
  ...
  it("matches folder type", () => {
    ...
    expect(
      checkCommandMatch(folderItem, "", [
        makeFile("mydir", "application/x-directory"),
      ]),
    ).toBe(true);
  });
  ...
  it("respects min/max file count", () => {
    ...
    expect(
      checkCommandMatch(multiFileItem, "", [
        makeFile("a.txt"),
        makeFile("b.txt"),
      ]),
    ).toBe(true);
    expect(checkCommandMatch(multiFileItem, "", [makeFile("a.txt")])).toBe(
      false,
    );
  });
...
describe("getMatchedCommands", () => {
  const items = [
    ...
    makeItem("Image", [
      { type: "image", name: "Image", description: "", extensions: [".png"] },
    ]),
  ];
  ...
  it("filters by files", () => {
    const result = getMatchedCommands(items, "", [
