...
import { startColorPickerFlow } from '../colorPicker';
...
function fireColorPickerResult(payload: unknown) {
  const cb = mockListen.mock.calls[0][1] as (
...
describe('startColorPickerFlow', () => {
  ...
  afterEach(async () => {
    vi.useRealTimers();
    // Reset module-level isPicking for next test
    try {
  ...
  it('calls beforeStart hook', async () => {
    ...
    startColorPickerFlow({ beforeStart });
  ...
  it('registers listener for color_picker_result and starts picker', async () => {
    ...
    expect(mockListen).toHaveBeenCalledWith('color_picker_result', expect.any(Function));
  ...
  it('shows error toast when clipboard write fails', async () => {
    mockInvoke.mockImplementation(async (cmd: string) => {
      if (cmd === 'start_color_picker') return;
      if (cmd === 'plugin_clipboard_write_text') throw new Error('clipboard error');
      if (cmd === 'close_main_window') return;
    });
    await startColorPickerFlow();
    await fireColorPickerResult({
  ...
  it('shows toast overlay when useToastOverlay is true', async () => {
    ...
    window.setTimeout = ((fn: () => void) => { fn(); }) as never;
  ...
  it('shows error toast overlay when clipboard fails and useToastOverlay is true', async () => {
    ...
    window.setTimeout = orig;
    expect(mockShowToastOverlay).toHaveBeenCalledWith('#333 已取色，复制失败', {
      kind: 'error',
      duration: 1800,
    });
  });
